#line 1 "c:\\Users\\jingtaoz\\Documents\\_School\\UCLA\\ECE202A\\ecem202a_project\\software\\Project\\Ard\\arduino_inference\\filtfilt.cpp"
#include <iostream>
#include <algorithm>
#include <exception>
#include <vector>
...
void append_vector(vectord &vec, const vectord &tail) { vec.insert(vec.end(), tail.begin(), tail.end()); }
...
inline int max_val(const vectori &vec) { return std::max_element(vec.begin(), vec.end())[0]; }

void filter(vectord B, vectord A, const vectord &X, vectord &Y, vectord &Zi) {
    if (A.empty()) {
        Serial.println("The feedback filter coefficients are empty.");
        //throw std::domain_error("The feedback filter coefficients are empty.");
    }
    if (std::all_of(A.begin(), A.end(), [](double coef) { return coef == 0; })) {
        Serial.println("At least one of the feedback filter coefficients has to be non-zero.");
        //throw std::domain_error("At least one of the feedback filter coefficients has to be non-zero.");
    }
    if (A[0] == 0) {
        Serial.println("First feedback coefficient has to be non-zero.");
...
void filtfilt(vectord B, vectord A, const vectord &X, vectord &Y) {
    ...
    if (len <= nfact) {
        ...
        Serial.println("Input data too short! Data must have length more than 3 times filter order.");
    }
    ...
    //Calculate initial conditions usign scipy
    vectord zzi_2 = {0.47237561749805684, 0.6495443503353961, 0.24956446486739686};

    //Do the forward and backward filtering
    y0 = signal1[0];

    zi.resize(zzi_2.size());

    std::transform(zzi_2.begin(), zzi_2.end(), zi.begin(), [y0](double val) { return val * y0; });
