...
logger.debug(f"y_train.shape={y_train.shape} y_test.shape={y_test.shape}")
...
with open(os.path.join(CUR_DIR, "configs/default.json"), "r") as f:
    ...
    logger.debug(f"dcl_params={dcl_params}")
...
for fold_id, (train_index, valid_index) in enumerate(cv.split(X_train, y_train)):
    ...
    # debug_idx = np.asarray([0,14,39,77,124])
    # X_tr_sub = X_train[debug_idx,:]
    # y_tr_sub = y_train[debug_idx]
    # np.save('X_debug.npy', X_tr_sub)
    # np.save('y_debug.npy', y_tr_sub)
